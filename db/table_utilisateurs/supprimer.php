...
// Vérifier qu'un utilisateur ne tente pas de se supprimer lui-même
if ($id == $_SESSION['nom_prenom']) {
    header('Location: ../index.php?message=Vous ne pouvez pas supprimer votre propre compte');
    exit;
}
...
if ($result->num_rows === 0) {
    header('Location: ../../admin/index.php?message=Utilisateur non trouvé');
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['confirmer']) && $_POST['confirmer'] === 'oui') {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: ../../admin/index.php?message=Utilisateur supprimé avec succès");
            exit;
        } else {
...
<body>
    ...
    <div class="confirmation-box">
        <div class="user-details">
            ...
            <p><strong>Nom et prénom :</strong> <?= htmlspecialchars($utilisateur['nom_prenom']) ?></p>
            <p><strong>Email :</strong> <?= htmlspecialchars($utilisateur['email']) ?></p>
            <p><strong>Rôle :</strong> <?= ($utilisateur['role'] == 'admin') ? 'adminnicien' : 'Utilisateur standard' ?></p>
        </div>
